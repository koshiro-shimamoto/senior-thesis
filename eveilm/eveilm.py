...
@click.command()
@click.option("--file", "file_path", type=click.Path(exists=True), help="EVM bytecode file")
def main(file_path):
    if file_path:
        file_path = Path(file_path).name
        contract = analyze_evmcode_file(file_path)
        selected_obf_type = choose_obfuscation_type(auto=True)
    else:
